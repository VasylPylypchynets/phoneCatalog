...
const Menu: React.FC<MenuProps> = ({ isMenuOpen, onCloseMenu }) => {
  const { favouritesLength } = useSelector(
    (state: RootState) => state.favourites,
  );
  const { cardsLength } = useSelector((state: RootState) => state.cards);
  ...
  return (
    ...
    >
      ...
      <header className={styles.menu__header}>
        <div className={styles['menu__header-container']}>
          <div className={styles['menu__header-left']}>
            <NavLink
              to={'/'}
              className={styles['menu__header-logo-link']}
              onClick={onCloseMenu}
            >
              <img
                src="./img/logo/logo.png"
                alt="logo"
                className={styles['menu__header-logo-img']}
              />
            </NavLink>
          </div>
          <div className={styles['menu__header-buttons']}>
            <div
              onClick={onCloseMenu}
              className={`${styles['menu__header-buttons-right']} ${styles['header__close-bth']}`}
            >
              <svg
                width="16"
                height="16"
      ...
      <div className={styles.menu__bottom}>
        ...
        <div className={styles.menu__buttons}>
          <NavLink
            to="/favorites"
            className={`${styles['menu__buttons-down']} ${styles.menu__favorites}`}
            onClick={onCloseMenu}
          >
            <div className={styles.menu__numbers}>
              <div
                className={classNames(styles['menu__numbers-count'], {
                  [styles['menu__numbers-count--active']]: favouritesLength > 0,
                })}
              >
                {favouritesLength}
              </div>
          ...
          <NavLink
            to="/card"
            className={`${styles['menu__buttons-down']} ${styles['menu__shopping-bag']}`}
            onClick={onCloseMenu}
          >
            <div className={styles.menu__numbers}>
              <div
                className={classNames(styles['menu__numbers-count'], {
                  [styles['menu__numbers-count--active']]: cardsLength > 0,
                })}
              >
                {cardsLength}
              </div>
